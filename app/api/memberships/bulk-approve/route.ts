...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const rl = rateLimit({ limit: 5, windowMs: 10_000, key: `${clerkUserId}:bulk-approve` });
      ...
      // Only admins/owners can bulk approve
      ...
      const result = await withGUC(client, {
        ...
        // Update memberships to approved status
      ...
      await withGUC(client, {
        ...
        `, [
          ...
          JSON.stringify({
            ...
            approved_user_ids: result.map(r => r.user_id),
            by: org?.role
          })
        ]);
      });
    ...
    console.error("POST /api/memberships/bulk-approve error", err);
    return NextResponse.json(
